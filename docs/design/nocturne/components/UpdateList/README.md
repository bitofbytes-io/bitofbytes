...
- An `nc-card` with class `nc-updates`: an uppercase `nc-card__title` "Latest updates" and an `nc-live` count ("6 in the last 30 days": projects whose `LastUpdate` is within the rolling 30-day `RecentWindow`, not the calendar month; omit it when zero).
