...
- Body: `row-title` name linking to the detail page, followed by a Badge "Updated this month" when `LastUpdate` is within 30 days; the tagline (`ink-soft` 17px); the "Latest ·" line (mono `accent` lead-in, then the note in `ink-subtle`); up to four Tech chips.
